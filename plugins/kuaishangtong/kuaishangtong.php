...
class kuaishangtong extends pluginBase
{
	...
	public function showFloat($seller_id = 0)
	{
		...
		if(!$content || !$content['float'] || !IFilter::act($content['float'],'url'))
		{
			return;
		}
	...
	public function online_edit()
	{
		...
		if($seller_id)
		{
			$this->redirect('seller_online_edit',array("content" => $data));
		}
		else
		{
			$this->redirect('system_online_edit',array("content" => $data));
		}
	}
